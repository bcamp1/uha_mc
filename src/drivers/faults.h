 ...
 *     the two tension arms, the roller encoder, and an RS485 comms-fault flag.
 *     faults_check_health() evaluates the sensor bits from the main loop; the
 *     RS485 bit is driven by faults_note_motor_reply().
 *
...
#define CTRL_FAULT_RS485           (1u << 3)  // a motor stopped responding on RS485 (debounced)

// When 1, any active fault disarms the deck (motors disabled, motion inhibited)
// until a clean re-arm via CMD_PLAY. Set to 0 to keep faults purely report-only
// -- useful when debugging faults without losing motor drive.
#define DISARM_ON_FAULT 1
...
//   - err != RX_ERR_OK            -> grow the failure streak; after the debounce
//                                    threshold mark offline and raise CTRL_FAULT_RS485
//                                    (the motor's last-known byte is held)
...
// True when any fault byte is non-zero -- the controller meta byte (tension /
// roller / RS485) or any per-motor fault byte -- AND DISARM_ON_FAULT is enabled.
// The main-loop supervisor uses this to disarm and to refuse re-arm while a fault
// persists. Always false when DISARM_ON_FAULT is 0.
